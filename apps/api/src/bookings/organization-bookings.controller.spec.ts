...
const service = { findByOrganization } as unknown as BookingsService;

function handler(): object {
  const descriptor = Object.getOwnPropertyDescriptor(
    OrganizationBookingsController.prototype,
    'findByOrganization',
  );
  if (!descriptor) {
...
function contextFor(request: object): ExecutionContext {
  return {
    getHandler: handler,
    getClass: () => OrganizationBookingsController,
    switchToHttp: () => ({ getRequest: () => request }),
  } as unknown as ExecutionContext;
}

describe('OrganizationBookingsController', () => {
  const controller = new OrganizationBookingsController(service);
