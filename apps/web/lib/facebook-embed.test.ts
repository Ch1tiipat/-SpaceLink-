...
const facebookAssert: typeof import('node:assert/strict') = require('node:assert/strict');
const { test: facebookTest }: typeof import('node:test') = require('node:test');
const { classifyFacebookUrl, getFacebookEmbeddedPost } =
  require('./facebook-embed.ts') as typeof import('./facebook-embed');
...
facebookTest(
  'builds an official plugin URL for the approved public post',
  () => {
...
facebookTest(
  'does not embed a Facebook Page URL because it is not a post',
  () => {
    facebookAssert.equal(
...
facebookTest(
  'rejects lookalike domains, plugin URLs, and unsafe schemes',
  () => {
    const rejectedUrls = [
...
facebookTest(
  'classifies the approved public post for the Admin preview',
  () => {
...
facebookTest(
  'classifies supported Facebook Page URLs without claiming an embed',
  () => {
    facebookAssert.deepEqual(
      ...
      {
        kind: 'page',
        sourceUrl: 'https://www.facebook.com/profile.php?id=61594183376080',
      },
    );
    facebookAssert.deepEqual(
