...
filterTest(
  'serializes compact bookmarks and preserves non-default pagination',
  () => {
    ...
    filterAssert.equal(
      params.toString(),
      'tab=refunds&refundStatus=PENDING&q=%E0%B8%84%E0%B8%B7%E0%B8%99%E0%B9%80%E0%B8%87%E0%B8%B4%E0%B8%99&page=2&pageSize=50',
    );
  },
);
...
filterTest('round trips a valid filter set', () => {
  ...
  filterAssert.deepEqual(
    parseAdminTransactionFilters(serializeAdminTransactionFilters(expected)),
    expected,
  );
});
