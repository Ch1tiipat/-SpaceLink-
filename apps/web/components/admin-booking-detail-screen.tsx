...
export function AdminBookingDetailScreen({ bookingId }: { bookingId: string }) {
  ...
  const [detail, setDetail] = useState<AdminTransactionBookingDetail | null>(
    null,
  );
      ...
      <AdminPage>
        ...
        {!loading && !error && !detail ? (
          <AdminPanel className="mt-6">
            <AdminEmpty
              icon={ReceiptText}
              title="ไม่พบรายละเอียดการจอง"
              description="รายการอาจไม่มีอยู่หรือไม่ได้อยู่ในองค์กรที่เลือก"
            />
          </AdminPanel>
        ) : null}
    ...
    <>
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <AdminMetric
          icon={ReceiptText}
          label="สถานะการจอง"
          value={BOOKING_LABELS[booking.status]}
        />
        <AdminMetric
          icon={CircleDollarSign}
          label="สถานะชำระเงิน"
          value={PAYMENT_LABELS[payment.status]}
          tone={
            payment.status === 'VERIFIED' || payment.status === 'EXEMPT'
              ? 'green'
        ...
        <AdminMetric
          icon={Clock3}
          label="Milestone ที่บันทึกได้"
          value={detail.timeline.length}
          tone="blue"
        />
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.8fr)]">
        <div className="grid content-start gap-6">
          <AdminPanel title="ข้อมูลการจองและพื้นที่">
            <dl className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Detail
                label="ราคาบูธ"
                value={formatAdminMoney(booking.boothPrice)}
              />
              <Detail label="สถานะ" value={BOOKING_LABELS[booking.status]} />
              <Detail
                label="อีเวนต์"
                value={detail.event.name}
                icon={CalendarDays}
              />
              <Detail
                label="โซน"
                value={`${detail.zone.code} · ${detail.zone.name || 'ไม่ระบุชื่อ'}`}
                icon={MapPinned}
              />
              <Detail label="บูธ" value={detail.booth.code} />
              <Detail
                label="วันเริ่มจอง"
                value={formatAdminDate(booking.bookingStartDate)}
              />
              <Detail
                label="วันสิ้นสุดจอง"
                value={formatAdminDate(booking.bookingEndDate)}
              />
              <Detail
                label="สร้างเมื่อ"
              ...
              {booking.confirmedAt ? (
                <Detail
                  label="ยืนยันเมื่อ"
                  value={formatAdminDateTime(booking.confirmedAt)}
                />
              ) : null}
              {booking.cancelledAt ? (
                <Detail
                  label="ยกเลิกเมื่อ"
                  value={formatAdminDateTime(booking.cancelledAt)}
                />
              ) : null}
              {booking.cancelReason ? (
                <Detail label="เหตุผลยกเลิก" value={booking.cancelReason} />
              ) : null}
            </dl>
          </AdminPanel>
          ...
          <AdminPanel title="ผู้จองและร้านค้า">
            <div className="grid gap-4 p-5 sm:grid-cols-2">
              ...
              <section className="rounded-2xl border border-[#e8e1ee] bg-[#fcfbff] p-4">
                ...
                <p className="mt-1 text-sm text-muted">
                  ร้านค้าที่ใช้ในการจองรายการนี้
                </p>
              </section>
            </div>
          </AdminPanel>
          ...
          <AdminPanel
            title="การชำระเงิน"
            description="สถานะคำนวณจากข้อมูลที่บันทึกจริงในระบบ"
          >
            <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">
              <Detail label="สถานะ" value={PAYMENT_LABELS[payment.status]} />
              <Detail
                label="เวลาที่มีผล"
                value={formatAdminDateTime(payment.effectiveAt)}
              />
              <Detail
                label="ช่องทาง"
                value={
                  booking.isPaymentExempt
                    ? 'ผู้ดูแลยกเว้นชำระเงิน'
                    : payment.group
                      ? 'ชำระรวมหลาย Booking'
                      : 'ชำระราย Booking'
                }
              />
              {booking.paymentExemptReason ? (
                <Detail
                  label="เหตุผลยกเว้น"
                  value={booking.paymentExemptReason}
                />
              ) : null}
              {payment.group ? (
                <>
                  <Detail
                    label="รหัสกลุ่มชำระ"
                    value={payment.group.paymentCode}
                  />
                  <Detail
                    label="ยอดรวมกลุ่ม"
                    value={formatAdminMoney(payment.group.totalAmount)}
                  />
                  <Detail label="สถานะกลุ่ม" value={payment.group.status} />
                </>
              ) : null}
            </div>
            {!booking.isPaymentExempt ? (
              <div className="border-t border-[#eee9f3] px-5 py-4">
        ...
        <AdminPanel
          ...
          description="Persisted milestone timeline จาก timestamp ที่ระบบบันทึกไว้ ไม่ใช่ immutable audit trail ทุก transition"
          className="h-fit"
        >
          <ol className="p-5">
            {detail.timeline.map((item, index) => {
              ...
              return (
                <li
                  key={`${item.type}-${item.entityId}-${item.timestamp}`}
                  className="relative flex gap-3 pb-6 last:pb-0"
                >
                  <span
                    className={`relative z-[1] mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full border text-[11px] font-black ${TIMELINE_TONES[description.tone]}`}
                  >
                    {index + 1}
                  </span>
                  {index < detail.timeline.length - 1 ? (
                    <span className="absolute left-[13px] top-7 h-[calc(100%-1.25rem)] w-px bg-[#e4ddec]" />
                  ) : null}
                  <div>
                    ...
                    {description.detail ? (
                      <p className="mt-1 text-xs font-bold text-[#655d70]">
                        {description.detail}
                      </p>
                    ) : null}
                  </div>
...
function SlipTable({
  ...
  slips: AdminTransactionBookingDetail['payment']['slips'];
}) {
  if (slips.length === 0)
    return (
      <p className="border-t border-[#eee9f3] px-5 py-5 text-sm text-muted">
        ยังไม่มีประวัติการส่งสลิป
      </p>
    );
  return (
    <div className="overflow-x-auto border-t border-[#eee9f3]">
      <table className="w-full min-w-[760px] text-left text-sm">
        <thead className="bg-[#faf8fc] text-xs text-muted">
          <tr>
            <th className="px-5 py-3">แหล่ง</th>
            <th className="px-4 py-3">สถานะ</th>
            <th className="px-4 py-3">ยอด</th>
            <th className="px-4 py-3">ธนาคาร / เลขอ้างอิง</th>
            <th className="px-4 py-3">เวลา</th>
          </tr>
        </thead>
        <tbody>
          {slips.map((slip) => (
            <tr key={slip.id} className="border-t border-[#eee9f3]">
              <td className="px-5 py-4 font-bold">
                {slip.source === 'PAYMENT_GROUP' ? 'สลิปกลุ่ม' : 'สลิป Booking'}
              </td>
              <td className="px-4 py-4">{slip.status}</td>
              <td className="px-4 py-4 font-extrabold">
                {formatAdminMoney(slip.amount)}
              </td>
              <td className="px-4 py-4 text-xs text-muted">
                {slip.sendingBank || '—'} · {slip.transRef || 'ไม่มีเลขอ้างอิง'}
              </td>
              <td className="px-4 py-4 text-xs text-muted">
                {formatAdminDateTime(slip.verifiedAt ?? slip.createdAt)}
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    <AdminPanel title="คำร้องคืนเงิน" description={`${refunds.length} รายการ`}>
      {refunds.length === 0 ? (
        <AdminEmpty
          icon={RotateCcw}
          title="ไม่มีคำร้องคืนเงิน"
          description="Booking นี้ยังไม่เคยส่งคำร้องคืนเงิน"
        />
      ) : (
  ...
  return (
    <div>
      <dt className="flex items-center gap-1.5 text-xs font-bold text-muted">
        {Icon ? <Icon className="h-3.5 w-3.5 text-violet" aria-hidden /> : null}
        {label}
      </dt>
    ...
    <div className="mt-6 grid gap-4">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }, (_, index) => (
          <div key={index} className="skeleton h-32 rounded-[18px]" />
        ))}
      </div>
