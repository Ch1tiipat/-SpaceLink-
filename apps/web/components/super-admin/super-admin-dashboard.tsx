  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] px-[15px] pb-11 pt-[23px] sm:px-[34px] sm:pt-[31px]">
      ...
      {broadcastNotice ? (
        <p
          role="status"
          className="mt-3 rounded-[11px] border border-emerald-200 bg-emerald-50 px-3.5 py-3 text-sm font-bold text-emerald-800"
        >
          {broadcastNotice}
        </p>
      ) : null}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <span className="mt-1 block leading-relaxed">
              GET /organizations, /admins, /bookings, /refunds,
              /support-tickets, /penalties และ /audit-logs
            </span>
          </div>
        </div>
        <Link
          href="/super-admin/organizations"
          className="shrink-0 font-extrabold text-[#6d28d9]"
        >
          ดูองค์กรทั้งหมด →
        </Link>
      </section>
      ...
      <section
        className="mt-[18px] grid gap-3.5 sm:grid-cols-2 xl:grid-cols-4"
      ...
      >
        ...
        <MetricCard
          label="ACTIVE"
          value={counts.ACTIVE}
          detail={percentage(counts.ACTIVE, organizations.length)}
          tone="green"
          icon={Check}
          loading={loading}
        />
        <MetricCard
          ...
          detail={percentage(counts.INACTIVE, organizations.length)}
          tone="orange"
          icon={Minus}
          loading={loading}
        />
        <MetricCard
          ...
          detail={percentage(counts.SUSPENDED, organizations.length)}
          tone="red"
          icon={AlertTriangle}
          loading={loading}
        />
      ...
      <section
        className="mt-3 grid gap-3.5 sm:grid-cols-2 xl:grid-cols-4"
        aria-label="ตัวเลขการดำเนินงาน"
      >
        <MetricCard
          label="องค์กรที่ใช้งาน"
          value={counts.ACTIVE}
          detail={`จากทั้งหมด ${organizations.length} องค์กร`}
          tone="green"
          icon={Building2}
          loading={loading}
        />
        <MetricCard
          ...
          detail={`จากทั้งหมด ${bookings.length} รายการ`}
          tone="blue"
          icon={CalendarCheck2}
          loading={loading}
        />
        <MetricCard
          label="คำร้องคืนเงินรอตรวจ"
          value={pendingRefunds}
          detail={`จากทั้งหมด ${refunds.length} คำร้อง`}
          tone="orange"
          icon={CircleDollarSign}
          loading={loading}
        />
        <MetricCard
          label="Support ที่ยังไม่ปิด"
          value={openTickets}
          detail={`จากทั้งหมด ${tickets.length} คำร้อง`}
          tone="red"
          icon={TicketCheck}
          loading={loading}
        />
      ...
      <section className="mt-[18px] grid gap-[18px] xl:grid-cols-[minmax(0,1.55fr)_minmax(310px,.85fr)]">
        <article className="overflow-hidden rounded-2xl border border-[#e5dcf0] bg-white shadow-[0_16px_38px_rgba(74,48,112,.06)]">
          <header className="flex flex-col gap-3 border-b border-[#eee8f4] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div
              className="flex rounded-lg border border-[#e7dfea] bg-[#fbf8ff] p-1"
              aria-label="เลือกรูปแบบกราฟ"
            >
              <ChartButton
                active={chartMode === 'donut'}
        ...
        <article className="overflow-hidden rounded-2xl border border-[#e5dcf0] bg-white shadow-[0_16px_38px_rgba(74,48,112,.06)]">
          <header className="flex items-center justify-between border-b border-[#eee8f4] px-5 py-4">
            ...
            <Link
              href="/super-admin/organizations"
              className="text-xs font-extrabold text-[#6d28d9]"
            >
              ทั้งหมด →
            </Link>
          </header>
          <div className="divide-y divide-[#f0ecf3] px-5">
            <AttentionRow
              label="SUSPENDED"
              description="ตรวจสอบผลกระทบก่อนเปิดกลับ"
              value={counts.SUSPENDED}
              tone="red"
            />
            <AttentionRow
              label="INACTIVE"
              description="องค์กรที่ปิดใช้งานอยู่"
              value={counts.INACTIVE}
              tone="orange"
            />
            <AttentionRow
              label="บทลงโทษ"
              description="รายการกำกับดูแลข้ามองค์กร"
              value={penalties.penalties.length}
              tone="red"
              icon={ShieldAlert}
            />
            <AttentionRow
              label="ผู้ใช้ Blacklist"
              description="ผู้ใช้ที่ถูกระงับทั่วทั้งระบบ"
              value={penalties.blacklistedUsers.length}
              tone="red"
              icon={ShieldAlert}
            />
            <AttentionRow
              label="แอดมินบริษัท"
              description="ข้อมูลข้ามทุกองค์กร"
              value={admins.length}
              tone="purple"
            />
          </div>
                  ...
                  {watchlist.map((organization) => (
                    ...
                    >
                      <td className="px-2 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            ...
                            <small className="mt-0.5 block max-w-[180px] truncate text-[10px] text-[#82788b]">
                              {organization.id}
                            </small>
                          </div>
                        </div>
                      ...
                      <td className="px-2 py-3 text-right font-extrabold">
                        {adminCounts.get(organization.id) ?? 0}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <article className="overflow-hidden rounded-2xl border border-[#e5dcf0] bg-white shadow-[0_16px_38px_rgba(74,48,112,.06)]">
          ...
          {loading ? (
            ...
            <EmptyPanel text="ยังไม่มีกิจกรรมในระบบ" />
          ) : (
            <div className="divide-y divide-[#f0ecf3] px-5">
              {auditLogs.slice(0, 5).map((log) => (
                <div key={log.id} className="py-3.5">
                  <div className="flex items-start justify-between gap-3">
                    <strong className="text-xs text-[#423b4c]">
                      {humanizeAction(log.action)}
                    </strong>
                    <time
                      dateTime={log.createdAt}
                      className="shrink-0 text-[10px] text-[#948a98]"
                    >
                      {THAI_DATE_TIME.format(new Date(log.createdAt))}
                    </time>
                  </div>
                  <p className="mt-1 truncate text-[11px] text-[#82788b]">
...
function SystemBroadcastDialog({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : 'ส่งประกาศระบบไม่สำเร็จ',
      );
    } finally {
      setSending(false);
    }
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2
              id="system-broadcast-title"
              className="mt-1 text-xl font-black text-[#242032]"
            >
              ส่งประกาศระบบ
            </h2>
    ...
    <article className="flex items-center gap-3.5 overflow-hidden rounded-[14px] border border-[#e7def4] bg-[linear-gradient(145deg,#fff,#fbf8ff)] p-[17px] shadow-[0_10px_26px_rgba(74,48,112,.05)] transition hover:-translate-y-0.5 hover:border-[#d9c7ef] hover:shadow-[0_17px_35px_rgba(74,48,112,.1)]">
      ...
      <div>
        ...
        {loading ? (
          ...
          <strong className="mt-0.5 block text-[24px] leading-tight text-[#242032]">
            {value.toLocaleString('th-TH')}
          </strong>
        )}
        <small className={`text-[11px] font-bold ${TONES[tone].detail}`}>
          {detail}
        </small>
      </div>
    </article>
...
function ChartButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`inline-flex min-h-8 items-center gap-1.5 rounded-md px-2.5 text-[11px] font-bold transition ${active ? 'bg-white text-[#6d28d9] shadow-sm' : 'text-[#82788b]'}`}
    >
...
function StatusChart({
  ...
  mode: ChartMode;
  counts: Record<SuperAdminOrganizationStatus, number>;
  total: number;
}) {
  ...
  if (mode === 'donut') {
    return (
      <div className="grid w-full gap-6 sm:grid-cols-[190px_minmax(0,1fr)] sm:items-center">
        <div
          className="relative mx-auto grid h-[190px] w-[190px] place-items-center rounded-full shadow-[0_14px_30px_rgba(124,58,237,.16)]"
          style={{
            background: `conic-gradient(#7c3aed 0 ${activeEnd}%,#e29c36 ${activeEnd}% ${inactiveEnd}%,#d14343 ${inactiveEnd}% 100%)`,
          }}
        >
          <div className="grid h-[120px] w-[120px] place-items-center rounded-full bg-white text-center">
            <div>
              <strong className="block text-[25px]">{total}</strong>
              <span className="text-[11px] text-[#82788b]">องค์กรทั้งหมด</span>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2.5">
          <ChartSummary label="ACTIVE" value={counts.ACTIVE} />
          <ChartSummary label="INACTIVE" value={counts.INACTIVE} />
          <ChartSummary label="SUSPENDED" value={counts.SUSPENDED} />
          <ChartSummary label="TOTAL" value={total} />
        </div>
      </div>
    );
  }
  ...
  if (mode === 'bar') {
    return (
      <div className="flex h-[220px] w-full items-end justify-around gap-5 border-b border-[#ebe6f0] px-4">
        {values.map((item) => (
          <div
            key={item.label}
            className="grid h-full flex-1 grid-rows-[28px_1fr_28px] items-end text-center"
          >
            <strong className="text-xs text-[#62576c]">{item.value}</strong>
            <div className="relative mx-auto h-full w-[min(58px,72%)] overflow-hidden rounded-t-[9px] bg-[#f2edf8]">
              <span
                className="absolute inset-x-0 bottom-0 rounded-t-[9px]"
                style={{
            ...
            <small className="self-center text-[10px] text-[#82788b]">
              {item.label}
            </small>
          </div>
        ))}
  ...
  const points = values
    .map(
      (item, index) => `${80 + index * 250},${220 - (item.value / max) * 160}`,
    )
    .join(' ');
  return (
    <svg
      ...
      role="img"
      aria-label="กราฟเส้นจำนวนองค์กรตามสถานะ"
    >
      <g stroke="#eee8f4" strokeWidth="1">
...
function ChartSummary({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-[10px] border border-[#eee8f4] bg-white p-3">
      <span className="block text-[11px] text-[#82788b]">{label}</span>
      <strong className="mt-1 block text-[17px]">{value}</strong>
    </div>
  );
}
...
function AttentionRow({
  ...
  return (
    <div className="flex items-center gap-3 py-4">
      <span
        className={`grid h-8 w-8 place-items-center rounded-[9px] ${TONES[tone].icon}`}
      >
        <Icon className="h-4 w-4" />
      </span>
      <div className="min-w-0 flex-1">
        <strong className="block text-xs text-[#423b4c]">{label}</strong>
        <small className="mt-1 block truncate text-[11px] text-[#82788b]">
          {description}
        </small>
      </div>
      <b className="text-sm text-[#242032]">{value}</b>
      <span className="text-[#948a98]">›</span>
    </div>
  );
}
function OrganizationMark({ name }: { name: string }) {
  return (
    <span className="grid h-8 w-8 shrink-0 place-items-center rounded-[9px] bg-[#f1eaff] text-xs font-extrabold text-[#6d28d9]">
...
function StatusPill({ status }: { status: SuperAdminOrganizationStatus }) {
  ...
  return (
    <span
      className={`inline-block rounded-md px-2 py-1 text-[10px] font-extrabold ${styles[status]}`}
    >
      {status}
    </span>
  );
  ...
  return (
    <div className="grid gap-3 p-5">
      {[1, 2, 3].map((item) => (
        <div
          key={item}
          className="h-10 animate-pulse rounded-lg bg-[#f2edf8]"
        />
      ))}
    </div>
  );
...
function ChartSkeleton() {
  return (
    <div className="h-[190px] w-[190px] animate-pulse rounded-full bg-[#f2edf8]" />
  );
}
function EmptyPanel({ text }: { text: string }) {
  return (
    <div className="grid min-h-[150px] place-items-center p-6 text-center text-xs text-[#82788b]">
      {text}
    </div>
  );
}
function countStatuses(organizations: SuperAdminOrganization[]) {
  return organizations.reduce<Record<SuperAdminOrganizationStatus, number>>(
    (result, organization) => {
...
function statusPriority(status: SuperAdminOrganizationStatus) {
  return status === 'SUSPENDED' ? 0 : status === 'INACTIVE' ? 1 : 2;
}
function percentage(value: number, total: number) {
  return total === 0
    ? '0% ของทั้งหมด'
    : `${((value / total) * 100).toFixed(1)}% ของทั้งหมด`;
}
