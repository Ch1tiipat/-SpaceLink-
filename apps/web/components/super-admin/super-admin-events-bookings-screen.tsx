  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (active)
          setOrganizationsError(
            errorMessage(cause, 'โหลดรายชื่อองค์กรไม่สำเร็จ'),
          );
      } finally {
    ...
    void (async () => {
      try {
        ...
        if (cause instanceof DOMException && cause.name === 'AbortError')
          return;
        if (active)
          setBookingsError(errorMessage(cause, 'โหลดข้อมูลการจองไม่สำเร็จ'));
      } finally {
    ...
    void (async () => {
      try {
        ...
        if (cause instanceof DOMException && cause.name === 'AbortError')
          return;
        if (active)
          setRefundsError(errorMessage(cause, 'โหลดข้อมูลคืนเงินไม่สำเร็จ'));
      } finally {
    ...
    void (async () => {
      try {
        ...
        if (cause instanceof DOMException && cause.name === 'AbortError')
          return;
        if (active)
          setEventsError(errorMessage(cause, 'โหลดรายการ Event ไม่สำเร็จ'));
      } finally {
...
function BookingsTab({
  ...
  const filtered = useMemo(() => {
    ...
    return bookings.filter((booking) => {
      ...
      const matchesOrganization =
        organizationId === 'ALL' ||
        booking.event.organization.id === organizationId;
      const matchesQuery =
        !normalized ||
        [
          booking.bookingCode,
  ...
  const hasFilters =
    query.trim().length > 0 || status !== 'ALL' || organizationId !== 'ALL';
  ...
  return (
      ...
      controls={
        <div className="grid gap-3 lg:grid-cols-[minmax(260px,1fr)_220px_240px_auto]">
          <SearchInput
            value={query}
            onChange={setQuery}
            placeholder="ค้นหารหัสจอง Event ร้าน ผู้จอง หรือบูธ"
          />
          <Select
            value={status}
    ...
    >
      {loading ? (
        ...
        <StatePanel
          title={
            hasFilters ? 'ไม่พบการจองที่ตรงกับตัวกรอง' : 'ยังไม่มีการจองในระบบ'
          }
          detail={
            hasFilters
              ? 'ลองเปลี่ยนคำค้นหาหรือตัวกรอง'
              : 'รายการจะแสดงเมื่อมีการสร้าง Booking'
          }
        />
      ) : (
        <PagedTable
            ...
            <>
              <thead className="bg-[#faf7fd] text-[11px] font-extrabold uppercase tracking-[.45px] text-[#82788b]">
                <tr>
                  ...
                  <th className="px-3 py-3">Event / องค์กร</th>
                  <th className="px-3 py-3">ร้าน / ผู้จอง</th>
                  <th className="px-3 py-3">บูธ</th>
                  <th className="px-3 py-3">ราคา</th>
                  <th className="px-3 py-3">สถานะ</th>
                  <th className="px-3 py-3">หลักฐาน</th>
              ...
              <tbody>
                {visibleRows.map((booking) => (
                  ...
                  >
                    <td className="px-5 py-4 font-black text-[#242032]">
                      {booking.bookingCode}
                    </td>
                    <td className="px-3 py-4">
                      ...
                      <span className="mt-1 inline-flex items-center gap-1 text-xs text-[#82788b]">
                    ...
                    <td className="px-3 py-4">
                      ...
                      <span className="block text-[10px] text-[#9a91a2]">
                        {booking.vendor.email}
                      </span>
                    </td>
                    <td className="px-3 py-4">
                      ...
                      <span className="text-xs text-[#82788b]">
                        โซน {booking.booth.zone.name || booking.booth.zone.code}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 font-extrabold text-[#423b4c]">
...
function PaymentsTab({
  ...
  const [status, setStatus] = useState<'ALL' | SuperAdminRefund['status']>(
    'ALL',
  );
  const [page, setPage] = useState(1);
  ...
  return (
      ...
      controls={
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <Select
            value={status}
            onChange={(value) =>
              setStatus(value as 'ALL' | SuperAdminRefund['status'])
            }
    ...
    >
      {loading ? (
        ...
        <StatePanel
          title={
            status === 'ALL' ? 'ยังไม่มีคำร้องคืนเงิน' : 'ไม่พบคำร้องสถานะนี้'
          }
          detail={
            status === 'ALL'
              ? 'รายการจะแสดงเมื่อผู้ขายส่งคำร้องคืนเงิน'
              : 'ลองเลือกสถานะอื่นหรือแสดงทุกสถานะ'
          }
        />
      ) : (
        <PagedTable
            ...
            <>
              <thead className="bg-[#faf7fd] text-[11px] font-extrabold uppercase tracking-[.45px] text-[#82788b]">
                <tr>
                  ...
                  <th className="px-3 py-3">Event / องค์กร</th>
                  <th className="px-3 py-3">ร้าน / ผู้ขอ</th>
                  <th className="px-3 py-3">เหตุผล</th>
                  <th className="px-3 py-3">ยอดที่ขอ</th>
                  <th className="px-3 py-3">ยอดอนุมัติ</th>
                  <th className="px-3 py-3">สถานะ</th>
                  <th className="px-3 py-3">สลิปต้นทาง</th>
              ...
              <tbody>
                {visibleRows.map((refund) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-4">
                      <strong className="block text-[#242032]">
                        {refund.booking.event.name}
                      </strong>
                      <span className="mt-1 inline-flex items-center gap-1 text-xs text-[#82788b]">
                        <Building2 className="h-3.5 w-3.5" />
                        {refund.booking.event.organization.name}
                      </span>
                    </td>
                    <td className="px-3 py-4">
                      <strong className="block text-[#423b4c]">
                        {refund.booking.shop.name}
                      </strong>
                      <span className="mt-1 block text-xs text-[#82788b]">
                    ...
                    <td className="max-w-[260px] px-3 py-4 text-xs leading-5 text-[#62576c]">
                      {refund.reason}
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 font-extrabold text-[#423b4c]">
                    ...
                    <td className="whitespace-nowrap px-3 py-4 font-extrabold text-[#423b4c]">
                      {refund.approvedAmount === null
                        ? '—'
                        : formatMoney(refund.approvedAmount)}
                    </td>
                    <td className="px-3 py-4">
                      <RefundStatusPill status={refund.status} />
                    </td>
                    <td className="px-3 py-4">
                      <AdminSlipActions bookingId={refund.bookingId} />
                    </td>
                    <td className="whitespace-nowrap px-5 py-4 text-xs text-[#716675]">
...
function EventsTab({
  ...
  return (
    ...
    >
      {!selectedOrganizationId ? (
        <StatePanel
          title="เลือกองค์กรเพื่อดูรายการ Event"
          detail="ระบบจะไม่เรียกข้อมูล Event จนกว่าจะเลือกองค์กร"
        />
      ) : loading ? (
        <TableSkeleton />
      ) : error ? (
        ...
        <StatePanel
          title="องค์กรนี้ยังไม่มี Event"
          detail={`ยังไม่พบ Event ของ ${selectedOrganization?.name ?? 'องค์กรที่เลือก'}`}
        />
      ) : (
        <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3">
          {events.map((event) => (
            <article
              key={event.id}
              className="rounded-[14px] border border-[#e7dfea] bg-[linear-gradient(145deg,#fff,#fcf9ff)] p-4 shadow-[0_8px_22px_rgba(65,43,85,.045)]"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {event.subscription ? (
                  <SubscriptionStatusPill status={event.subscription.status} />
                ) : (
                  <span className="rounded-md bg-[#f1eef4] px-2 py-1 text-[10px] font-extrabold text-[#655d70]">
                    ไม่มี Subscription
                  </span>
                )}
              </div>
              <h3 className="mb-1 mt-4 text-base font-black text-[#242032]">
                {event.name}
              </h3>
              <p className="m-0 inline-flex items-center gap-1.5 text-xs text-[#716675]">
                <Store className="h-3.5 w-3.5" />
                {event.venue.name}
              </p>
              <dl className="mt-4 grid gap-2 border-t border-[#eee8f2] pt-3 text-xs">
                <div className="flex justify-between gap-3">
                  <dt className="text-[#82788b]">วันที่จัด</dt>
                  <dd className="m-0 text-right font-bold text-[#4f4658]">
                    {formatDate(event.startDate)} – {formatDate(event.endDate)}
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#82788b]">สถานะ Event</dt>
                  <dd className="m-0 font-bold text-[#4f4658]">
                    {eventStatusLabel(event.status)}
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#82788b]">ค่าบริการ</dt>
                  <dd className="m-0 font-black text-[#6d28d9]">
                    {event.subscription
                      ? formatMoney(event.subscription.finalPrice)
                      : '—'}
                  </dd>
                </div>
              </dl>
...
function BookingStatusPill({ status }: { status: BookingStatus }) {
  const styles: Record<BookingStatus, string> = {
    ...
    NO_SHOW: 'bg-[#f1eef4] text-[#655d70]',
    COMPLETED: 'bg-[#eaf2ff] text-[#2459b5]',
  };
  return (
    <span
...
function RefundStatusPill({ status }: { status: SuperAdminRefund['status'] }) {
  const styles: Record<SuperAdminRefund['status'], string> = {
...
function SubscriptionStatusPill({
  ...
  status: NonNullable<AdminOrganizationEvent['subscription']>['status'];
}) {
  const styles = {
    DRAFT: 'bg-[#f1eef4] text-[#655d70]',
    PENDING_PAYMENT: 'bg-[#fff4df] text-[#9a570f]',
    ACTIVE: 'bg-[#e7f8ef] text-[#147653]',
    EXPIRED: 'bg-[#fff0ef] text-[#b42318]',
    CANCELLED: 'bg-[#fff0ef] text-[#b42318]',
  } as const;
  const labels = {
    DRAFT: 'ฉบับร่าง',
  ...
  return (
    <span
      className={`rounded-md px-2 py-1 text-[10px] font-extrabold ${styles[status]}`}
    >
      {labels[status]}
    </span>
  );
