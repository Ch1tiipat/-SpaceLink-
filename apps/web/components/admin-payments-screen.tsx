...
const PAYMENT_STYLES: Record<AdminPaymentStatus, string> = {
  EXEMPT: 'bg-[#edf4ff] text-[#2563eb]',
  AWAITING_SLIP: 'bg-[#fff4df] text-[#9a570f]',
  VERIFIED: 'bg-[#e7f8ef] text-[#147653]',
...
export function AdminPaymentsScreen({
  ...
  return (
    <AdminPanel
      ...
      description="หนึ่งแถวต่อ Booking; รายการชำระรวมจะแสดงยอดบูธและยอดรวมแยกกัน"
    >
      {error ? (
        ...
        <AdminEmpty
          ...
          title="ไม่พบบันทึกการชำระเงิน"
          description="ลองเปลี่ยนตัวกรองสถานะหรือช่วงวันที่"
        />
      ) : (
          ...
          <table className="w-full min-w-[1080px] border-collapse text-left text-sm">
            <thead className="bg-[#faf8fc] text-[11px] text-muted">
              <tr>
                ...
                <th className="px-4 py-3">อีเวนต์</th>
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <strong className="block">{item.shop.name}</strong>
                    <span className="text-xs text-muted">
                      {item.vendor.fullName}
                    </span>
                  </td>
                  <td className="px-4 py-4">{item.event.name}</td>
                  <td className="px-4 py-4">
                    <strong className="block">
                      บูธ {formatAdminMoney(item.boothPrice)}
                    </strong>
                    {item.paymentGroup ? (
                      <span className="mt-1 block text-xs font-bold text-violet">
                        ชำระรวม {item.paymentGroup.paymentCode} · ยอดกลุ่ม{' '}
                        {formatAdminMoney(item.paymentGroup.totalAmount)}
                      </span>
                    ) : (
                      <span className="text-xs text-muted">
                        ชำระราย Booking
                  ...
                  <td className="px-4 py-4">
                    <span
                      className={`rounded-full px-2.5 py-1 text-[11px] font-extrabold ${PAYMENT_STYLES[item.paymentStatus]}`}
                    >
                  ...
                  <td className="px-4 py-4">
                    {item.paymentStatus === 'EXEMPT' ? (
                      ...
                      <AdminSlipActions bookingId={item.id} />
                    )}
                  </td>
                  <td className="px-4 py-4 text-xs text-muted">
                    {formatAdminDateTime(item.paymentEffectiveAt)}
                  </td>
                </tr>
              ))}
            </tbody>
...
export function AdminRefundsScreen({
  ...
  return (
    <AdminPanel
      title="คำร้องคืนเงิน"
      description="แสดงคำร้องจริง ไม่ใช่การอนุมานจากสถานะ Booking"
    >
      {error ? (
        <AdminError message={error} />
      ) : loading ? (
            ...
            <tbody>
              {items.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <strong className="block">{item.booking.shop.name}</strong>
                    <span className="text-xs text-muted">
                      {item.booking.vendor.fullName}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <strong className="block">
                      ขอ {formatAdminMoney(item.requestedAmount)}
                    </strong>
                    <span className="text-xs text-muted">
                      อนุมัติ{' '}
                      {item.approvedAmount
                        ? formatAdminMoney(item.approvedAmount)
                        : '—'}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <span
                      className={`rounded-full px-2.5 py-1 text-[11px] font-extrabold ${REFUND_STYLES[item.status]}`}
                    >
                      {item.status}
                    </span>
