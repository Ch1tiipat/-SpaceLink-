...
import {
  AdminPaymentsScreen,
  AdminRefundsScreen,
} from '@/components/admin-payments-screen';
...
const TABS: Array<{ value: AdminTransactionTab; label: string }> = [
  ...
  { value: 'refunds', label: 'คืนเงิน' },
  { value: 'vendors', label: 'ผู้ขาย/ร้านค้า' },
];
...
export function AdminTransactionsScreen() {
  ...
  const filters = useMemo(
    () => parseAdminTransactionFilters(searchParams),
    [searchParams],
  );
  const [queryDraft, setQueryDraft] = useState(filters.q);
  const [response, setResponse] = useState<AdminTransactionResponse | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const query: AdminTransactionQuery = {
      ...
      bookingStatus:
        (filters.bookingStatus as AdminTransactionQuery['bookingStatus']) ||
        undefined,
      paymentStatus:
        (filters.paymentStatus as AdminTransactionQuery['paymentStatus']) ||
        undefined,
      refundStatus:
        (filters.refundStatus as AdminTransactionQuery['refundStatus']) ||
        undefined,
    ...
    void getAdminTransactions(organizationId, query, token, controller.signal)
      .then((value) => {
  ...
  function update(next: Partial<AdminTransactionFilters>) {
    ...
    router.replace(`${pathname}?${serializeAdminTransactionFilters(merged)}`, {
      scroll: false,
    });
  }
  ...
  const totalPages = response
    ...
    : 1;
  const bookingItems =
    response?.view === 'BOOKINGS' || response?.view === 'PAYMENTS'
      ? (response.items as AdminTransactionBooking[])
      : [];
  const refundItems =
    response?.view === 'REFUNDS'
      ? (response.items as AdminTransactionRefund[])
      : [];
  const vendorItems =
    response?.view === 'VENDORS'
      ? (response.items as AdminTransactionVendor[])
      : [];
  ...
  return (
    <AdminAccessGate access={access}>
      <AdminPage>
        ...
        />
        <nav
          aria-label="หมวดการเงินและการจอง"
          className="mt-6 flex gap-2 overflow-x-auto rounded-2xl border border-[#e8e1ee] bg-white p-2"
        >
          {TABS.map((tab) => (
            <button
              key={tab.value}
              ...
              className={`min-h-10 shrink-0 rounded-xl px-4 text-sm font-extrabold ${filters.tab === tab.value ? 'bg-violet text-white' : 'text-[#655d70] hover:bg-[#f7f2ff]'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <AdminPanel
          className="mt-4"
          title="ตัวกรอง"
          description="ค่าตัวกรองจะอยู่ใน URL เพื่อเปิดซ้ำหรือส่งต่อได้"
        >
          <form
            onSubmit={submitQuery}
            className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-4"
          >
            <label className="flex items-center gap-2 rounded-xl border border-[#ddd4e7] px-3 xl:col-span-2">
              <Search className="h-4 w-4 text-violet" aria-hidden />
              <input
                value={queryDraft}
                onChange={(event) => setQueryDraft(event.target.value)}
                maxLength={100}
                placeholder="ค้นหารหัสจอง ผู้ขาย ร้าน อีเวนต์ หรือกลุ่มชำระ"
                className="h-10 min-w-0 flex-1 bg-transparent text-sm outline-none"
              />
              <button
                className="text-xs font-extrabold text-violet"
                type="submit"
              >
                ค้นหา
              </button>
            ...
            <FilterSelect
              ...
              options={(response?.filters.events ?? []).map((item) => [
                item.id,
                item.name,
              ])}
            />
            <FilterSelect
              ...
              options={(response?.filters.zones ?? []).map((item) => [
                item.id,
                item.name || item.code,
              ])}
            />
            <FilterSelect
              ...
              onChange={(value) => update({ vendorUserId: value })}
              options={(response?.filters.vendors ?? []).map((item) => [
                item.id,
                item.fullName,
              ])}
            />
            <FilterSelect
              ...
              onChange={(value) => update({ shopId: value })}
              options={(response?.filters.shops ?? []).map((item) => [
                item.id,
                item.name,
              ])}
            />
            <FilterSelect
              ...
              options={BOOKING_STATUS.slice(1)}
              first={BOOKING_STATUS[0][1]}
            />
            <FilterSelect
              label="สถานะชำระ"
              value={filters.paymentStatus}
              onChange={(value) => update({ paymentStatus: value })}
              options={PAYMENT_STATUS.slice(1)}
              first={PAYMENT_STATUS[0][1]}
            />
            <FilterSelect
              label="สถานะคืนเงิน"
              value={filters.refundStatus}
              onChange={(value) => update({ refundStatus: value })}
              options={REFUND_STATUS.slice(1)}
              first={REFUND_STATUS[0][1]}
            />
            <label className="text-xs font-bold text-muted">
              ...
              <input
                ...
                className="mt-1 h-10 w-full rounded-xl border border-[#ddd4e7] bg-white px-3 text-sm text-ink"
              />
            </label>
            <label className="text-xs font-bold text-muted">
              ถึง
            ...
            <button
              ...
              onClick={() => {
                setQueryDraft('');
                router.replace(`${pathname}?tab=${filters.tab}`, {
                  scroll: false,
                });
              }}
              className="h-10 self-end rounded-xl border border-[#ddd4e7] px-3 text-xs font-extrabold text-muted"
            >
              ล้างตัวกรอง
            </button>
          </form>
        ...
        <div className="my-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <AdminMetric
            icon={Ticket}
            label="Booking ในผลลัพธ์"
            value={summary?.bookings ?? '—'}
          />
          <AdminMetric
            icon={CircleDollarSign}
            label="ตรวจสอบชำระแล้ว"
            value={summary?.payments.VERIFIED ?? '—'}
            tone="green"
          />
          <AdminMetric
            icon={RotateCcw}
            label="คืนเงินรอตรวจ"
            value={summary?.refunds.PENDING ?? '—'}
            tone="amber"
          />
          <AdminMetric
            icon={Store}
        ...
        <div className="mt-4 flex items-center justify-between gap-3 text-xs font-bold text-muted">
          <span>
            {response
              ? `${response.total} รายการ · หน้า ${response.page} จาก ${totalPages}`
              : 'กำลังเตรียมข้อมูล'}
          </span>
          <div className="flex gap-2">
            <button
              ...
              className="h-9 rounded-xl border border-[#ddd4e7] bg-white px-3 disabled:opacity-40"
            >
              ก่อนหน้า
            </button>
            <button
