...
export function AdminReviewsScreen() {
    ...
    getOrganizationReviews(
      ...
      .catch((cause: unknown) => {
        ...
        if (active) {
          setResult(null);
          setError(
            cause instanceof Error ? cause.message : 'โหลดข้อมูลรีวิวไม่สำเร็จ',
          );
        }
      })
      .finally(() => {
  ...
  const average = useMemo(() => {
    ...
    return (
      result.items.reduce((sum, review) => sum + review.rating, 0) /
      result.items.length
    );
  }, [result]);

  function setFilter(
    key: 'event' | 'status' | 'rating' | 'page',
    value: string,
  ) {
  ...
  async function submitModeration() {
    ...
    try {
      ...
      setActionError(
        cause instanceof Error ? cause.message : 'ดำเนินการไม่สำเร็จ',
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <AdminAccessGate access={access}>
      <AdminPage>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          <AdminMetric
            icon={Star}
            label="คะแนนเฉลี่ยในหน้านี้"
            value={average === null ? '—' : average.toFixed(1)}
            tone="amber"
          />
          <AdminMetric
            icon={MessageSquareText}
            label="รีวิวตามตัวกรอง"
            value={result?.total ?? 0}
          />
          <AdminMetric
            icon={EyeOff}
            label="ซ่อนอยู่ในหน้านี้"
            value={
              result?.items.filter((item) => item.status === 'HIDDEN').length ??
              0
            }
            tone="blue"
          />
        ...
        <AdminPanel
          className="mt-6"
          title="ตัวกรองรีวิว"
          description="ตัวกรองจะถูกเก็บใน URL เพื่อเปิดกลับมาดูชุดเดิมได้"
        >
          <div className="grid gap-3 p-5 md:grid-cols-3">
            <label className="text-sm font-bold">
              ...
              >
                ...
                {result?.filters.events.map((event) => (
        ...
        {!loading && !error && result?.items.length === 0 ? (
          <AdminPanel className="mt-6">
            <AdminEmpty
              icon={Star}
              title="ไม่พบรีวิว"
              description="ลองเปลี่ยนตัวกรอง หรือรอให้ผู้เข้าร่วมส่งรีวิวหลัง Event จบ"
            />
          </AdminPanel>
        ) : null}
        {!loading && result && result.items.length > 0 ? (
          <AdminPanel
            className="mt-6"
            title="รายการรีวิว"
            description="ชื่อผู้รีวิวถูกซ่อนเพื่อคงความเป็นส่วนตัว"
          >
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="border-b border-line bg-[#faf8fd] text-xs text-muted">
                  <tr>
                    <th className="px-5 py-3">ผู้รีวิว / ความคิดเห็น</th>
                    <th className="px-5 py-3">Event / บูธ</th>
                  ...
                  {result.items.map((review) => (
                    <tr key={review.id} className="align-top">
                      <td className="max-w-sm px-5 py-4">
                        <strong>ผู้ใช้ SpaceLink</strong>
                        <p className="mt-2 whitespace-pre-wrap break-words leading-6 text-muted">
                          {review.comment?.trim() ||
                      ...
                      <td className="px-5 py-4">
                        <strong>
                          {review.event?.name ?? 'ข้อมูล Event เดิม'}
                        </strong>
                        <p className="mt-1 text-muted">
                          {review.booking
                            ? `บูธ ${review.booking.booth.code} · ${review.booking.booth.zone.name ?? review.booking.booth.zone.code}`
                            : 'ไม่มีข้อมูลการจองเดิม'}
                        </p>
                      </td>
                      <td className="px-5 py-4 font-black text-[#9a6700]">
                        {review.rating}/5 ★
                      </td>
                      <td className="px-5 py-4">
                        <span className="rounded-full bg-[#f4efff] px-3 py-1 text-xs font-bold text-violet">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex min-w-40 flex-wrap gap-2">
                          {review.status === 'PUBLISHED' ? (
                            <button
                              type="button"
                              onClick={() => openModeration(review, 'hide')}
                              className="sl-chip"
                            >
                              <EyeOff className="h-4 w-4" aria-hidden /> ซ่อน
                            </button>
                          ) : null}
                          {review.status === 'HIDDEN' ? (
                            <button
                              type="button"
                              onClick={() => openModeration(review, 'restore')}
                              className="sl-chip"
                            >
                          ...
                          {review.status !== 'DELETED' ? (
                            <button
                              type="button"
                              onClick={() => openModeration(review, 'delete')}
                              className="sl-chip text-danger"
                            >
            ...
            <div className="flex items-center justify-between gap-3 border-t border-line p-5">
              <button
                ...
                className="sl-action-secondary disabled:opacity-40"
              >
                หน้าก่อนหน้า
              </button>
        ...
        {moderation ? (
          <div
            ...
            role="presentation"
            onMouseDown={() => !submitting && setModeration(null)}
          >
            <section
              role="dialog"
              aria-modal="true"
            ...
            >
              <h2 id="moderation-title" className="text-xl font-black">
              ...
              <p className="mt-2 text-sm leading-6 text-muted">
                การดำเนินการนี้จะบันทึกผู้ดำเนินการ เวลา เหตุผล
                และสถานะก่อน–หลังใน Audit Log
              </p>
              <label className="mt-5 block text-sm font-bold">
                ...
                <textarea
                  ...
                  placeholder="ระบุเหตุผลในการจัดการรีวิว"
                />
              </label>
              {actionError ? (
